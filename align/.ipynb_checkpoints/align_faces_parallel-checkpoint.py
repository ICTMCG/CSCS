...
def align_face(filepath, predictor):
	...
	# Save aligned image.
	return crop, img, quad+.5, pad, if_pad # i forget why +.5
...
def extract_on_paths(file_paths):
	...
	for file_path, res_path in file_paths:
		...
		try:
			crop, res, quad, pad, if_pad = align_face(file_path, predictor)
			res = res.convert('RGB')
			os.makedirs(os.path.dirname(res_path), exist_ok=True)
			res.save(res_path)

			np.save(res_path[:-4], crop)
			np.save(res_path[:-4]+'_quad', quad)
			if if_pad:
				np.save(res_path[:-4]+'_pad', pad)
...
def run(args):
	...
	# out_crops_path = root_path.replace(dir_name, f'{dir_name}_ffhq_align')
	out_crops_path = args.output
	...
	for file in os.listdir(root_path):
		...
		res_path = os.path.join(out_crops_path, file)
