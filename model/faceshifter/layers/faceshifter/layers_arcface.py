...
class AADLayer(nn.Module):
    ...
    def forward(self, h_in, z_attr, z_id):
        ...
        # iteract tgt and ID with attention
...
class AADGenerator(nn.Module):
    ...
    def forward(self, z_attr, z_id):
        m = self.up1(z_id.reshape(z_id.shape[0], -1, 1, 1)) # 生成图像是由z_id up出来的
...
if __name__ == "__main__":
    aie = AEI_Net(512)
    x = aie(torch.randn(1, 3, 256, 256), torch.randn(1, 512))
